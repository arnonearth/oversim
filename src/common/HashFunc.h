...
template<> struct hash<TransportAddress> : std::unary_function<TransportAddress, std::size_t>
{
    ...
    {
        if (addr.getAddress().isIPv6()) {
            return ((bswap_32(addr.getAddress().get6().words()[0]) ^
                     bswap_32(addr.getAddress().get6().words()[1]) ^
                     bswap_32(addr.getAddress().get6().words()[2]) ^
                     bswap_32(addr.getAddress().get6().words()[3])) ^
                      addr.getPort());
        } else {
            return (bswap_32(addr.getAddress().get4().getInt()) ^
                    addr.getPort());
        }
    }
};
