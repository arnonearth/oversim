...
void BaseOverlay::initialize(int stage)
{
    ...
    if (stage == MIN_STAGE_OVERLAY) {
        ...
        thisNode.setAddress(IPAddressResolver().
...
{
    EV << "[BaseOverlay::bindToPort() @ " << thisNode.getAddress()
...
void BaseOverlay::callUpdate(const NodeHandle& node, bool joined)
{
    if ((!node.isUnspecified()) && (node != thisNode)) {
        if (joined) {
            EV << "[BaseOverlay::callUpdate() @ " << thisNode.getAddress()
               << " (" << thisNode.getKey().toString(16) << ")]\n"
               << "    (" << node << ", " << joined << ") joined"
               << endl;
        } else {
...
void BaseOverlay::handleMessage(cMessage* msg)
{
    if (msg->getArrivalGate() == udpGate) {
        ...
        // debug message
        if (debugOutput) {
            EV << "[BaseOverlay:handleMessage() @ " << thisNode.getAddress()
            << " (" << thisNode.getKey().toString(16) << ")]\n"
            << "    Received " << *msg << " from "
            << overlayCtrlInfo->getLastHop().getAddress() << endl;
        }
        ...
        //std::cout << baseOverlayMsg->hasBitError()<<std::endl;
        ...
        if (overlayCtrlInfo->getLastHop().getAddress() == thisNode.getAddress()) {
    ...
    case OVERLAYROUTE: {
        ...
        if ((sourceRoute.size() == 0) &&
            ...
            if (isMalicious() && dropRouteMessageAttack) {
                EV << "[BaseOverlay::handleBaseOverlayMessage() @ " << thisNode.getAddress()
                << " (" << thisNode.getKey().toString(16) << ")]\n"
...
{
    // get new ip address
    thisNode.setAddress(IPAddressResolver().addressOf(
                      getParentModule()->getParentModule()));
                                   ...
                                   bool ack)
{
    ...
    if (ctrlInfo && ctrlInfo->getLastHop().getAddress() != thisNode.getAddress()) {
                                   ...
                                   cPacket* msg)
{
    ...
    udpControlInfo->setSrcAddr(thisNode.getAddress());
    ...
    if (dest != thisNode) {
        ...
        if (dest.getAddress() == thisNode.getAddress()) { // to same node, but different port
                        ...
                        RoutingType routingType)
{
    ...
    // debug output
    if (debugOutput) {
        EV << "[BaseOverlay::route() @ " << thisNode.getAddress()
        << " (" << thisNode.getKey().toString(16) << ")]\n"
                            ...
                            RoutingType routingType)
{
    ...
    if (debugOutput) {
        EV << "[BaseOverlay::sendToKey() @ " << thisNode.getAddress()
        << " (" << thisNode.getKey().toString(16) << ")]\n"
        << "    Sending " << msg <<  " to " << key
        << endl;
    }
    ...
    if ((routingType == ITERATIVE_ROUTING)
        ...
        if (nextHops->size() == 0) {
            EV << "[BaseOverlay::sendToKey() @ " << thisNode.getAddress()
               ...
               << endl;
            //std::cout << simTime() << " " << thisNode.getAddress() << " " << state
            //          << " FindNode() returned NULL - dropping message "
            ...
            if (routeMsg->getHopCount() >= hopCountMax) {
                ...
                EV << "[BaseOverlay::sendToKey() @ " << thisNode.getAddress()
                   ...
                   << routeMsg->getSrcNode().getAddress() << "\n"
                   << "    The hop count maximum has been exceeded ("
                   << routeMsg->getHopCount() << ">="
                   << hopCountMax << ")"
                   << endl;
                //std::cout << "[BaseOverlay::sendToKey() @ " << thisNode.getAddress()
                //          << " (" << thisNode.getKey().toString(16) << ")]\n"
                //          << "    Discards " << routeMsg->getName() << " from "
                //          << routeMsg->getSrcNode().getAddress() << "\n"
                //          << "    The hop count maximum has been exceeded ("
            ...
            if (nextHop == NULL) {
                if (!checkFindNode(routeMsg)) {
                    EV << "[BaseOverlay::sendToKey() @ " << thisNode.getAddress()
                       ...
                       << endl;
                    //std::cout << thisNode.getAddress() << " packet from "
                    //          << routeMsg->getSrcNode().getAddress()
                    //          << " dropped: " << routeMsg
                    //          << " " << state << std::endl;
                    RECORD_STATS(numDropped++;
...
void BaseOverlay::findNodeRpc( FindNodeCall* call )
{
    ...
    if (isMalicious() && dropFindNodeAttack) {
        EV << "[BaseOverlay::findNodeRpc() @ " << thisNode.getAddress()
           << " (" << thisNode.getKey().toString(16) << ")]\n"
