...
void BaseApp::handleMessage(cMessage* msg)
{
    ...
    if (msg->arrivedOn("from_lowerTier") ||
        ...
        if (debugOutput && !ev.isDisabled()) {
            ...
            EV << "[BaseApp:handleMessage() @ " << thisNode.getAddress()
                        ...
                        RoutingType routingType)
{
    ...
    // debug message
    if (debugOutput && !ev.isDisabled()) {
        EV << "[BaseApp::callRoute() @ " << thisNode.getAddress()
        << " (" << overlay->getThisNode().getKey().toString(16) << ")]\n"
        << "    Sending " << *msg
...
{
    EV << "[BaseApp::bindToPort() @ " << thisNode.getAddress()
...
void BaseApp::sendMessageToUDP(const TransportAddress& destAddr, cPacket *msg)
{
    ...
    ctrl->setSrcAddr(thisNode.getAddress());
    ctrl->setDestAddr(destAddr.getAddress());
    ...
    // debug message
    if (debugOutput) {
        EV << "[BaseApp::sendMessageToUDP() @ " << thisNode.getAddress()
        << " (" << overlay->getThisNode().getKey().toString(16) << ")]\n"
        << "    Sending " << *msg << " to " << destAddr.getAddress()
        << endl;
    }
